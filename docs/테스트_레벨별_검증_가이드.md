# 테스트 레벨별 검증 가이드

> 단위 테스트, 통합 테스트, E2E 테스트에서 각각 무엇을 검증해야 하는지에 대한 종합 가이드

## 목차

1. [테스트 피라미드 개요](#테스트-피라미드-개요)
2. [Unit Test (단위 테스트)](#1-unit-test-단위-테스트)
3. [Integration Test (통합 테스트)](#2-integration-test-통합-테스트)
4. [E2E Test (End-to-End 테스트)](#3-e2e-test-end-to-end-테스트)
5. [종합 비교](#종합-비교표)
6. [실전 권장사항](#실전-권장사항)

---

## 테스트 피라미드 개요

```
        /\
       /  \     E2E Tests (10%)
      /----\    - 사용자 시나리오
     /------\   - API 통신
    /--------\
   /----------\ Integration Tests (30%)
  /------------\ - 컴포넌트 통합
 /--------------\ - DB 연동
/----------------\ Unit Tests (60%)
 - 순수 로직
 - 개별 함수
```

### 핵심 원칙

- **빠른 테스트를 많이**: Unit Test가 가장 많아야 함
- **느린 테스트를 적게**: E2E Test는 핵심 시나리오만
- **중간 테스트를 적절히**: Integration Test로 통합 검증

---

## 1. Unit Test (단위 테스트)

### 📝 목적
**비즈니스 로직이 있는 함수/메서드를 검증 (의존성은 모킹)**

### 특징
- ⚡ **속도**: 매우 빠름 (밀리초 단위)
- 🔧 **의존성**: Mock/Stub 사용 (실제 DB/네트워크 사용 안함)
- 🎯 **범위**: Service 계층의 비즈니스 로직
- 📊 **비율**: 전체 테스트의 60%

### ⚠️ Repository는 단위 테스트가 부적합한 이유

**Repository 패턴**은 단순히 DB 접근 계층(Data Access Layer)입니다:
- ❌ **비즈니스 로직이 없음**: 단순히 Prisma 메서드를 호출만 함
- ❌ **과도한 모킹**: 입력도 모킹, 출력도 모킹 → 아무것도 검증 안 함
- ❌ **테스트 가치 없음**: "모킹된 값을 호출하면 모킹된 값이 반환된다"만 확인
- ✅ **통합 테스트가 적합**: 실제 DB를 사용해서 CRUD가 정상 작동하는지 검증

```typescript
// ❌ 나쁜 예: Repository 단위 테스트 (의미 없음)
describe('NotificationsRepository', () => {
  it('create 호출 시 모킹된 값 반환', async () => {
    const mockData = { userId: 'user-123', ... };
    const mockResult = { id: 'notif-1', ...mockData };

    // 입력도 모킹
    jest.spyOn(prisma.notification, 'create')
      .mockResolvedValue(mockResult); // 출력도 모킹

    const result = await repository.create(mockData);

    // 이게 뭘 테스트하는 건가? 🤔
    // "모킹하면 모킹된 값이 반환된다"만 확인하는 무의미한 테스트
    expect(result).toEqual(mockResult);
  });
});

// ✅ 올바른 접근: Repository는 통합 테스트로 (섹션 2 참고)
describe('NotificationsRepository 통합 테스트', () => {
  it('실제 DB에 저장하고 조회할 수 있다', async () => {
    // 실제 DB 사용
    const result = await repository.create({
      userId: testUserId,
      type: NotificationType.NEW_MESSAGE,
      title: '테스트',
      message: '메시지',
    });

    // 실제로 DB에 저장되었는지 확인
    const saved = await prisma.notification.findUnique({
      where: { id: result.id },
    });

    expect(saved).not.toBeNull();
    expect(saved.title).toBe('테스트');
  });
});
```

### ✅ Service 계층 단위 테스트 (적합)

**Service는 비즈니스 로직이 있으므로** 단위 테스트가 적합합니다:
- ✅ Repository를 모킹하고 Service 로직만 검증
- ✅ 의존성 호출 여부, 호출 인자, 반환값 검증
- ✅ 예외 처리, 조건 분기 로직 검증

#### Service 레이어 예시

```typescript
// notifications.service.spec.ts
describe('NotificationsService', () => {
  let service: NotificationsService;
  let repository: NotificationsRepository;

  beforeEach(() => {
    // Mock Repository
    repository = {
      create: jest.fn(),
      findMany: jest.fn(),
      markAsRead: jest.fn(),
      getUnreadCount: jest.fn(),
    } as any;

    service = new NotificationsService(repository);
  });

  describe('create', () => {
    it('Repository의 create를 호출한다', async () => {
      const mockData = {
        userId: 'user-123',
        type: NotificationType.NEW_MESSAGE,
        title: '테스트',
        message: '메시지',
      };
      const mockResult = { id: 'notif-1', ...mockData };

      (repository.create as jest.Mock).mockResolvedValue(mockResult);

      const result = await service.create(mockData);

      expect(repository.create).toHaveBeenCalledWith(mockData);
      expect(result).toEqual(mockResult);
    });
  });

  describe('findMyNotifications', () => {
    it('페이지네이션 결과를 반환한다', async () => {
      const mockResult = {
        items: [{ id: 'notif-1' }],
        total: 10,
        page: 1,
        totalPages: 1,
        unreadCount: 5,
      };

      (repository.findMany as jest.Mock).mockResolvedValue(mockResult);

      const result = await service.findMyNotifications('user-123', 1);

      expect(repository.findMany).toHaveBeenCalledWith('user-123', 1);
      expect(result).toEqual(mockResult);
    });
  });

  describe('markAsRead', () => {
    it('권한이 있으면 읽음 처리한다', async () => {
      (repository.markAsRead as jest.Mock).mockResolvedValue({ count: 1 });

      await service.markAsRead('notif-1', 'user-123');

      expect(repository.markAsRead).toHaveBeenCalledWith('notif-1', 'user-123');
    });

    it('권한이 없으면 0건 업데이트된다', async () => {
      (repository.markAsRead as jest.Mock).mockResolvedValue({ count: 0 });

      await service.markAsRead('notif-1', 'wrong-user');

      expect(repository.markAsRead).toHaveBeenCalledWith('notif-1', 'wrong-user');
    });
  });
});
```

### 📋 Unit Test 체크리스트

- [ ] 각 메서드가 올바른 인자로 의존성을 호출하는가?
- [ ] 반환값이 예상대로인가?
- [ ] 엣지 케이스(null, undefined, 빈 배열)를 처리하는가?
- [ ] 에러 케이스를 올바르게 처리하는가?
- [ ] 비즈니스 로직(계산, 변환)이 정확한가?

### ❌ Unit Test에서 하지 말아야 할 것

- 실제 DB 연결
- 네트워크 호출
- 파일 시스템 접근
- 외부 서비스 의존

---

## 2. Integration Test (통합 테스트)

### 📝 목적
**여러 컴포넌트가 함께 동작하는지 검증 (실제 DB 사용)**

### 특징
- ⚡ **속도**: 보통 (초 단위)
- 🔧 **의존성**: 실제 DB 사용 (테스트 DB)
- 🎯 **범위**: Repository + DB 또는 Service + Repository + DB
- 📊 **비율**: 전체 테스트의 30%

### ✅ Repository 통합 테스트 (권장)

**Repository는 통합 테스트로 작성하세요!**
- ✅ 실제 DB에 저장/조회/수정/삭제가 정상 작동하는지 검증
- ✅ Prisma 스키마의 제약조건(UNIQUE, NOT NULL 등) 검증
- ✅ 복잡한 쿼리(JOIN, 집계, 페이지네이션) 정확성 검증
- ✅ 트랜잭션 롤백 동작 검증

```typescript
// notifications.repository.integration.spec.ts
describe('NotificationsRepository 통합 테스트', () => {
  let repository: NotificationsRepository;
  let prisma: PrismaService;
  let testUserId: string;

  beforeAll(async () => {
    const module = await Test.createTestingModule({
      providers: [NotificationsRepository, PrismaService],
    }).compile();

    repository = module.get<NotificationsRepository>(NotificationsRepository);
    prisma = module.get<PrismaService>(PrismaService);
  });

  beforeEach(async () => {
    // 테스트 사용자 생성
    const user = await prisma.user.create({
      data: {
        email: `test-${Date.now()}@example.com`,
        password: 'hashed',
        nickname: '테스터',
      },
    });
    testUserId = user.id;
  });

  afterEach(async () => {
    // 테스트 데이터 정리
    await prisma.notification.deleteMany({ where: { userId: testUserId } });
    await prisma.user.delete({ where: { id: testUserId } });
  });

  describe('create', () => {
    it('알림을 DB에 저장하고 생성된 객체를 반환한다', async () => {
      // Given: 알림 생성 데이터
      const createData = {
        userId: testUserId,
        type: NotificationType.NEW_MESSAGE,
        title: '📣 테스트 알림',
        message: '알림 테스트입니다.',
      };

      // When: 알림 생성
      const result = await repository.create(createData);

      // Then: 반환된 객체 검증
      expect(result).toMatchObject({
        id: expect.any(String),
        userId: testUserId,
        type: NotificationType.NEW_MESSAGE,
        title: '📣 테스트 알림',
        message: '알림 테스트입니다.',
        isRead: false,
        readAt: null,
        createdAt: expect.any(Date),
      });

      // Then: 실제로 DB에 저장되었는지 확인
      const saved = await prisma.notification.findUnique({
        where: { id: result.id },
      });

      expect(saved).not.toBeNull();
      expect(saved.title).toBe('📣 테스트 알림');
    });

    it('relatedId와 relatedType을 포함한 알림을 생성한다', async () => {
      const createData = {
        userId: testUserId,
        type: NotificationType.ORDER_CONFIRMED,
        title: '주문 확인',
        message: '주문이 확인되었습니다.',
        relatedId: 'order-123',
        relatedType: 'Order',
      };

      const result = await repository.create(createData);

      expect(result.relatedId).toBe('order-123');
      expect(result.relatedType).toBe('Order');

      // DB 확인
      const saved = await prisma.notification.findUnique({
        where: { id: result.id },
      });
      expect(saved.relatedId).toBe('order-123');
    });
  });

  describe('findMany', () => {
    beforeEach(async () => {
      // 테스트용 알림 3개 생성
      await Promise.all([
        repository.create({
          userId: testUserId,
          type: NotificationType.NEW_MESSAGE,
          title: '알림1',
          message: '메시지1',
        }),
        repository.create({
          userId: testUserId,
          type: NotificationType.NEW_MESSAGE,
          title: '알림2',
          message: '메시지2',
        }),
        repository.create({
          userId: testUserId,
          type: NotificationType.NEW_MESSAGE,
          title: '알림3',
          message: '메시지3',
        }),
      ]);
    });

    it('페이지네이션이 정상 작동한다', async () => {
      // 1페이지 (limit: 2)
      const page1 = await repository.findMany(testUserId, 1, 2);

      expect(page1.items).toHaveLength(2);
      expect(page1.total).toBe(3);
      expect(page1.page).toBe(1);
      expect(page1.totalPages).toBe(2);

      // 2페이지
      const page2 = await repository.findMany(testUserId, 2, 2);

      expect(page2.items).toHaveLength(1);
      expect(page2.page).toBe(2);
    });

    it('다른 사용자의 알림은 조회되지 않는다', async () => {
      // 다른 사용자 생성
      const otherUser = await prisma.user.create({
        data: {
          email: `other-${Date.now()}@example.com`,
          password: 'hashed',
          nickname: '다른사용자',
        },
      });

      // 다른 사용자의 알림 생성
      await repository.create({
        userId: otherUser.id,
        type: NotificationType.NEW_MESSAGE,
        title: '다른 사용자 알림',
        message: '메시지',
      });

      // 내 알림만 조회됨
      const result = await repository.findMany(testUserId, 1, 10);

      expect(result.items).toHaveLength(3); // 내 알림 3개만
      expect(result.items.every(n => n.userId === testUserId)).toBe(true);

      // 정리
      await prisma.notification.deleteMany({ where: { userId: otherUser.id } });
      await prisma.user.delete({ where: { id: otherUser.id } });
    });
  });

  describe('markAsRead', () => {
    it('본인 알림을 읽음 처리할 수 있다', async () => {
      // 알림 생성
      const notification = await repository.create({
        userId: testUserId,
        type: NotificationType.NEW_MESSAGE,
        title: '테스트',
        message: '메시지',
      });

      expect(notification.isRead).toBe(false);

      // 읽음 처리
      const result = await repository.markAsRead(notification.id, testUserId);

      expect(result.count).toBe(1); // 1건 업데이트

      // DB 확인
      const updated = await prisma.notification.findUnique({
        where: { id: notification.id },
      });

      expect(updated.isRead).toBe(true);
    });

    it('다른 사용자의 알림은 읽음 처리할 수 없다', async () => {
      // 다른 사용자 생성
      const otherUser = await prisma.user.create({
        data: {
          email: `other-${Date.now()}@example.com`,
          password: 'hashed',
          nickname: '다른사용자',
        },
      });

      // 다른 사용자의 알림
      const notification = await repository.create({
        userId: otherUser.id,
        type: NotificationType.NEW_MESSAGE,
        title: '테스트',
        message: '메시지',
      });

      // 내가 읽음 처리 시도
      const result = await repository.markAsRead(notification.id, testUserId);

      // 권한 없음 - 0건 업데이트
      expect(result.count).toBe(0);

      // DB 확인 - 여전히 읽지 않음 상태
      const notUpdated = await prisma.notification.findUnique({
        where: { id: notification.id },
      });

      expect(notUpdated.isRead).toBe(false);

      // 정리
      await prisma.notification.delete({ where: { id: notification.id } });
      await prisma.user.delete({ where: { id: otherUser.id } });
    });
  });

  describe('delete', () => {
    it('본인 알림을 삭제할 수 있다', async () => {
      // 알림 생성
      const notification = await repository.create({
        userId: testUserId,
        type: NotificationType.NEW_MESSAGE,
        title: '테스트',
        message: '메시지',
      });

      // 삭제
      const deleted = await repository.delete(notification.id, testUserId);

      expect(deleted.id).toBe(notification.id);

      // DB 확인 - 삭제됨
      const notFound = await prisma.notification.findUnique({
        where: { id: notification.id },
      });

      expect(notFound).toBeNull();
    });

    it('다른 사용자의 알림은 삭제할 수 없다', async () => {
      // 다른 사용자 생성
      const otherUser = await prisma.user.create({
        data: {
          email: `other-${Date.now()}@example.com`,
          password: 'hashed',
          nickname: '다른사용자',
        },
      });

      // 다른 사용자의 알림
      const notification = await repository.create({
        userId: otherUser.id,
        type: NotificationType.NEW_MESSAGE,
        title: '테스트',
        message: '메시지',
      });

      // 내가 삭제 시도 - 에러 발생 예상
      await expect(
        repository.delete(notification.id, testUserId)
      ).rejects.toThrow();

      // DB 확인 - 여전히 존재
      const stillExists = await prisma.notification.findUnique({
        where: { id: notification.id },
      });

      expect(stillExists).not.toBeNull();

      // 정리
      await prisma.notification.delete({ where: { id: notification.id } });
      await prisma.user.delete({ where: { id: otherUser.id } });
    });
  });

  afterAll(async () => {
    await prisma.$disconnect();
  });
});
```

### ✅ Service + Repository + DB 통합 테스트

```typescript
// notifications.integration.spec.ts
describe('NotificationsModule Integration', () => {
  let module: TestingModule;
  let service: NotificationsService;
  let repository: NotificationsRepository;
  let prisma: PrismaService;

  beforeAll(async () => {
    module = await Test.createTestingModule({
      imports: [
        ConfigModule.forRoot(),
        PrismaModule,  // ✅ 실제 Prisma 사용
      ],
      providers: [NotificationsService, NotificationsRepository],
    }).compile();

    service = module.get(NotificationsService);
    repository = module.get(NotificationsRepository);
    prisma = module.get(PrismaService);
  });

  beforeEach(async () => {
    // 테스트 전 데이터 정리
    await prisma.notification.deleteMany();
  });

  describe('알림 생성 → 조회', () => {
    it('생성한 알림을 조회할 수 있다', async () => {
      // 1. Service를 통한 알림 생성
      const created = await service.create({
        userId: 'user-123',
        type: NotificationType.NEW_MESSAGE,
        title: '테스트',
        message: '메시지',
      });

      expect(created.id).toBeDefined();
      expect(created.userId).toBe('user-123');

      // 2. Repository를 통한 직접 조회
      const result = await repository.findMany('user-123', 1);

      // ✅ 생성된 알림이 조회되는지 검증
      expect(result.items).toHaveLength(1);
      expect(result.items[0].id).toBe(created.id);
    });
  });

  describe('읽음 처리', () => {
    it('본인 소유의 알림을 읽음 처리할 수 있다', async () => {
      // 알림 생성
      const notification = await service.create({
        userId: 'user-123',
        type: NotificationType.NEW_MESSAGE,
        title: '테스트',
        message: '메시지',
      });

      expect(notification.isRead).toBe(false);

      // 읽음 처리
      await service.markAsRead(notification.id, 'user-123');

      // ✅ DB에서 실제로 업데이트되었는지 확인
      const updated = await prisma.notification.findUnique({
        where: { id: notification.id },
      });

      expect(updated.isRead).toBe(true);
    });

    it('다른 사용자의 알림은 읽음 처리할 수 없다', async () => {
      // 사용자 A의 알림 생성
      const notification = await service.create({
        userId: 'user-A',
        type: NotificationType.NEW_MESSAGE,
        title: '테스트',
        message: '메시지',
      });

      // 사용자 B가 읽음 처리 시도
      const result = await service.markAsRead(notification.id, 'user-B');

      // ✅ 권한 검증
      expect(result.count).toBe(0);

      // DB에서 확인 - 여전히 읽지 않음 상태
      const notUpdated = await prisma.notification.findUnique({
        where: { id: notification.id },
      });

      expect(notUpdated.isRead).toBe(false);
    });
  });

  describe('페이지네이션', () => {
    it('20개씩 페이지네이션된다', async () => {
      // 25개 알림 생성
      const promises = Array.from({ length: 25 }, (_, i) =>
        service.create({
          userId: 'user-123',
          type: NotificationType.NEW_MESSAGE,
          title: `알림 ${i + 1}`,
          message: '메시지',
        })
      );

      await Promise.all(promises);

      // 1페이지 조회
      const page1 = await service.findMyNotifications('user-123', 1);

      // ✅ 페이지네이션 검증
      expect(page1.items).toHaveLength(20);
      expect(page1.total).toBe(25);
      expect(page1.totalPages).toBe(2);

      // 2페이지 조회
      const page2 = await service.findMyNotifications('user-123', 2);

      expect(page2.items).toHaveLength(5);
    });
  });

  describe('읽지 않은 알림 개수', () => {
    it('읽지 않은 알림 개수를 정확히 반환한다', async () => {
      // 5개 알림 생성
      const notifications = await Promise.all(
        Array.from({ length: 5 }, () =>
          service.create({
            userId: 'user-123',
            type: NotificationType.NEW_MESSAGE,
            title: '테스트',
            message: '메시지',
          })
        )
      );

      // 2개 읽음 처리
      await service.markAsRead(notifications[0].id, 'user-123');
      await service.markAsRead(notifications[1].id, 'user-123');

      // 읽지 않은 개수 확인
      const count = await service.getUnreadCount('user-123');

      // ✅ 정확한 개수 검증
      expect(count).toBe(3);
    });
  });

  afterAll(async () => {
    await prisma.$disconnect();
    await module.close();
  });
});
```

### 📋 Integration Test 체크리스트

- [ ] Service + Repository + DB 통합 동작하는가?
- [ ] 트랜잭션이 올바르게 동작하는가?
- [ ] 데이터 정합성이 유지되는가?
- [ ] 페이지네이션이 정확한가?
- [ ] 권한 검증이 실제 DB 레벨에서 동작하는가?
- [ ] 연관 데이터 처리가 정확한가?

### ❌ Integration Test에서 하지 말아야 할 것

- HTTP 요청/응답 테스트 (E2E의 영역)
- 모든 엣지 케이스 테스트 (Unit Test의 영역)
- 외부 API 호출 (Mock 처리)

---

## 3. E2E Test (End-to-End 테스트)

### 📝 목적
**실제 사용자 시나리오를 HTTP API를 통해 검증**

### 특징
- ⚡ **속도**: 느림 (초~분 단위)
- 🔧 **의존성**: 전체 애플리케이션 스택
- 🎯 **범위**: HTTP → Controller → Service → Repository → DB
- 📊 **비율**: 전체 테스트의 10%

### ✅ 검증 대상

```typescript
// notifications.e2e.spec.ts
describe('NotificationsController (E2E)', () => {
  let app: INestApplication;
  let prisma: PrismaService;
  let testUserToken: string;
  let testUserId: string;

  beforeAll(async () => {
    const moduleFixture = await Test.createTestingModule({
      imports: [AppModule],  // ✅ 전체 애플리케이션
    }).compile();

    app = moduleFixture.createNestApplication();

    // ✅ 실제 운영 환경과 동일하게 설정
    app.setGlobalPrefix('api/v1');
    app.useGlobalPipes(new ValidationPipe());
    app.useGlobalInterceptors(new TransformInterceptor());

    await app.init();

    prisma = app.get(PrismaService);

    // 테스트 사용자 생성 및 토큰 발급
    const user = await createTestUser(prisma);
    testUserId = user.id;
    testUserToken = generateJwtToken(user);
  });

  beforeEach(async () => {
    // 테스트 데이터 정리
    await prisma.notification.deleteMany({ where: { userId: testUserId } });
  });

  describe('POST /api/v1/notifications', () => {
    it('알림을 생성할 수 있다', async () => {
      const { body } = await request(app.getHttpServer())
        .post('/api/v1/notifications')
        .set('Authorization', `Bearer ${testUserToken}`)
        .send({
          userId: testUserId,
          type: NotificationType.NEW_MESSAGE,
          title: '📣 테스트 알림',
          message: '알림 테스트입니다.',
        })
        .expect(201);  // ✅ HTTP 상태 코드

      // ✅ 응답 구조 검증
      expect(body.success).toBe(true);
      expect(body.statusCode).toBe(201);
      expect(body.data).toMatchObject({
        title: '📣 테스트 알림',
        message: '알림 테스트입니다.',
        type: NotificationType.NEW_MESSAGE,
        isRead: false,
      });
      expect(body.data.id).toBeDefined();
      expect(body.timestamp).toBeDefined();
    });

    it('인증 없이 요청하면 401 에러', async () => {
      await request(app.getHttpServer())
        .post('/api/v1/notifications')
        .send({
          userId: testUserId,
          type: NotificationType.NEW_MESSAGE,
          title: '테스트',
          message: '메시지',
        })
        .expect(401);  // ✅ 인증 실패
    });

    it('필수 필드 누락 시 400 에러', async () => {
      await request(app.getHttpServer())
        .post('/api/v1/notifications')
        .set('Authorization', `Bearer ${testUserToken}`)
        .send({
          userId: testUserId,
          // type 누락
          title: '테스트',
        })
        .expect(400);  // ✅ 유효성 검증 실패
    });
  });

  describe('GET /api/v1/notifications', () => {
    it('내 알림 목록을 조회할 수 있다', async () => {
      // 사전 데이터 생성
      await createTestNotifications(prisma, testUserId, 3);

      const { body } = await request(app.getHttpServer())
        .get('/api/v1/notifications')
        .set('Authorization', `Bearer ${testUserToken}`)
        .expect(200);

      // ✅ 응답 구조 검증
      expect(body.success).toBe(true);
      expect(body.data.items).toHaveLength(3);
      expect(body.data.total).toBe(3);
      expect(body.data.page).toBe(1);
    });

    it('다른 사용자의 알림은 조회되지 않는다', async () => {
      // 다른 사용자의 알림 생성
      const otherUser = await createTestUser(prisma);
      await createTestNotifications(prisma, otherUser.id, 5);

      const { body } = await request(app.getHttpServer())
        .get('/api/v1/notifications')
        .set('Authorization', `Bearer ${testUserToken}`)
        .expect(200);

      // ✅ 권한 검증
      expect(body.data.items).toHaveLength(0);
    });
  });

  describe('PATCH /api/v1/notifications/:id/read', () => {
    it('알림을 읽음 처리할 수 있다', async () => {
      // 알림 생성
      const notification = await prisma.notification.create({
        data: {
          userId: testUserId,
          type: NotificationType.NEW_MESSAGE,
          title: '테스트',
          message: '메시지',
        },
      });

      await request(app.getHttpServer())
        .patch(`/api/v1/notifications/${notification.id}/read`)
        .set('Authorization', `Bearer ${testUserToken}`)
        .expect(200);

      // ✅ 실제 DB 확인
      const updated = await prisma.notification.findUnique({
        where: { id: notification.id },
      });

      expect(updated.isRead).toBe(true);
    });

    it('다른 사용자의 알림은 읽음 처리할 수 없다', async () => {
      // 다른 사용자의 알림
      const otherUser = await createTestUser(prisma);
      const notification = await prisma.notification.create({
        data: {
          userId: otherUser.id,
          type: NotificationType.NEW_MESSAGE,
          title: '테스트',
          message: '메시지',
        },
      });

      await request(app.getHttpServer())
        .patch(`/api/v1/notifications/${notification.id}/read`)
        .set('Authorization', `Bearer ${testUserToken}`)
        .expect(200);  // 200이지만 실제로 업데이트 안됨

      // ✅ DB 확인 - 여전히 읽지 않음 상태
      const notUpdated = await prisma.notification.findUnique({
        where: { id: notification.id },
      });

      expect(notUpdated.isRead).toBe(false);
    });
  });

  describe('사용자 시나리오: 알림 생애주기', () => {
    it('생성 → 조회 → 읽음 처리 → 개수 확인', async () => {
      // 1. 알림 생성
      const created = await request(app.getHttpServer())
        .post('/api/v1/notifications')
        .set('Authorization', `Bearer ${testUserToken}`)
        .send({
          userId: testUserId,
          type: NotificationType.NEW_MESSAGE,
          title: '새 메시지',
          message: '메시지가 도착했습니다',
        })
        .expect(201);

      const notificationId = created.body.data.id;

      // 2. 읽지 않은 개수 확인
      const unreadBefore = await request(app.getHttpServer())
        .get('/api/v1/notifications/unread/count')
        .set('Authorization', `Bearer ${testUserToken}`)
        .expect(200);

      expect(unreadBefore.body.data.unreadCount).toBe(1);

      // 3. 알림 목록 조회
      const list = await request(app.getHttpServer())
        .get('/api/v1/notifications')
        .set('Authorization', `Bearer ${testUserToken}`)
        .expect(200);

      expect(list.body.data.items).toContainEqual(
        expect.objectContaining({ id: notificationId, isRead: false })
      );

      // 4. 읽음 처리
      await request(app.getHttpServer())
        .patch(`/api/v1/notifications/${notificationId}/read`)
        .set('Authorization', `Bearer ${testUserToken}`)
        .expect(200);

      // 5. 읽지 않은 개수 다시 확인
      const unreadAfter = await request(app.getHttpServer())
        .get('/api/v1/notifications/unread/count')
        .set('Authorization', `Bearer ${testUserToken}`)
        .expect(200);

      expect(unreadAfter.body.data.unreadCount).toBe(0);
    });
  });

  afterAll(async () => {
    await prisma.$disconnect();
    await app.close();
  });
});
```

### 📋 E2E Test 체크리스트

- [ ] HTTP 상태 코드가 올바른가?
- [ ] 인증/인가가 작동하는가?
- [ ] 유효성 검증이 작동하는가?
- [ ] 응답 형식이 일관된가? (TransformInterceptor)
- [ ] 실제 사용자 시나리오가 동작하는가?
- [ ] 권한이 올바르게 분리되는가?
- [ ] 에러 케이스가 적절히 처리되는가?

### ❌ E2E Test에서 하지 말아야 할 것

- 모든 필드의 타입과 값 검증 (Unit Test의 영역)
- Prisma 스키마의 기본값 검증 (DB 레벨 테스트)
- 모든 엣지 케이스 (Unit Test의 영역)
- 세부 로직 검증 (Unit/Integration Test의 영역)

---

## 종합 비교표

| 항목 | Unit Test | Integration Test | E2E Test |
|------|-----------|------------------|----------|
| **속도** | 매우 빠름 (ms) | 보통 (초) | 느림 (초~분) |
| **범위** | 단일 함수/메서드 | 여러 컴포넌트 | 전체 시스템 |
| **의존성** | Mock | 실제 DB | 실제 전체 스택 |
| **검증 대상** | 로직 정확성 | 통합 동작 | 사용자 시나리오 |
| **비율** | 60% | 30% | 10% |
| **예시** | 계산 로직 | Service+Repository | HTTP API 호출 |
| **실행 환경** | 격리된 환경 | 테스트 DB | 전체 앱 스택 |
| **유지보수** | 쉬움 | 보통 | 어려움 |

---

## 실전 권장사항

### ✅ 각 레벨에서 **반드시** 검증할 것

#### Unit Test
- ✅ 비즈니스 로직 정확성
- ✅ 엣지 케이스 처리
- ✅ 의존성 호출 검증
- ✅ 반환값 정확성
- ✅ 에러 처리

#### Integration Test
- ✅ DB 트랜잭션
- ✅ 데이터 정합성
- ✅ 컴포넌트 간 통신
- ✅ 복잡한 쿼리 동작
- ✅ 권한 검증 (DB 레벨)

#### E2E Test
- ✅ HTTP 상태 코드
- ✅ 인증/인가
- ✅ 사용자 시나리오
- ✅ 응답 형식 일관성
- ✅ 실제 동작 검증

### 🎯 테스트 작성 우선순위

1. **Unit Test** 먼저 (빠르고 많이)
   - 모든 Repository 메서드
   - 모든 Service 메서드
   - 비즈니스 로직이 있는 함수

2. **Integration Test** 중간 (핵심 흐름)
   - CRUD 통합 시나리오
   - 트랜잭션이 필요한 경우
   - 복잡한 쿼리 동작

3. **E2E Test** 마지막 (주요 시나리오만)
   - 핵심 사용자 시나리오
   - 인증/인가 흐름
   - 주요 API 엔드포인트

### 💡 테스트 효율화 팁

#### 1. **과도한 검증 피하기**
```typescript
// ❌ 나쁜 예: E2E에서 모든 필드 검증
expect(body.data.id).toBeDefined();
expect(body.data.userId).toBe(testUserId);
expect(body.data.type).toBe('NEW_MESSAGE');
expect(body.data.title).toBe('📣 테스트 알림');
expect(body.data.message).toBe('알림 테스트입니다.');
expect(body.data.isRead).toBe(false);
expect(body.data.readAt).toBeNull();
expect(body.data.createdAt).toBeDefined();
// ... 이건 Unit Test에서 해야 할 일

// ✅ 좋은 예: E2E는 핵심만 검증
expect(body.success).toBe(true);
expect(body.data).toMatchObject({
  title: '📣 테스트 알림',
  message: '알림 테스트입니다.',
  type: NotificationType.NEW_MESSAGE,
  isRead: false,
});
expect(body.data.id).toBeDefined();
```

#### 2. **스냅샷 테스트 활용**
```typescript
// 많은 필드를 검증해야 한다면 스냅샷 사용
expect({
  ...body.data,
  id: expect.any(String),
  createdAt: expect.any(String),
}).toMatchSnapshot();
```

#### 3. **헬퍼 함수 활용**
```typescript
// 재사용 가능한 검증 함수
const expectValidNotification = (notification: any) => {
  expect(notification).toMatchObject({
    id: expect.any(String),
    userId: expect.any(String),
    type: expect.any(String),
    title: expect.any(String),
    message: expect.any(String),
    isRead: expect.any(Boolean),
    createdAt: expect.any(String),
  });
};

// 테스트에서 사용
it('알림 생성', async () => {
  const { body } = await request(app.getHttpServer())
    .post('/api/v1/notifications')
    .send({ ... })
    .expect(201);

  expectValidNotification(body.data);
  expect(body.data.title).toBe('📣 테스트 알림');
});
```

#### 4. **시나리오 중심 E2E 작성**
```typescript
// ❌ 나쁜 예: 개별 엔드포인트만 테스트
it('알림 생성', async () => { ... });
it('알림 조회', async () => { ... });
it('알림 읽음', async () => { ... });

// ✅ 좋은 예: 전체 시나리오 테스트
it('알림 생애주기: 생성 → 조회 → 읽음 처리 → 삭제', async () => {
  // 실제 사용자가 사용하는 흐름대로 테스트
});
```

### 📊 테스트 커버리지 목표

| 레벨 | 목표 커버리지 | 우선순위 |
|------|--------------|---------|
| Unit Test | 80-90% | 높음 |
| Integration Test | 60-70% | 중간 |
| E2E Test | 주요 시나리오만 | 낮음 |

### 🚀 테스트 실행 전략

```bash
# 개발 중: Unit Test만 빠르게 실행
npm test -- --testPathPattern=spec.ts

# 커밋 전: Integration Test까지 실행
npm test -- --testPathPattern="(spec|integration.spec).ts"

# CI/CD: 전체 테스트 실행
npm test

# 특정 모듈만 테스트
npm test -- notifications
```

---

## 결론

### 핵심 원칙

1. **빠른 테스트를 많이**: Unit Test 60%
2. **통합 테스트를 적절히**: Integration Test 30%
3. **느린 테스트를 적게**: E2E Test 10%

### 황금 규칙

- ✅ **Unit Test**: 로직 정확성
- ✅ **Integration Test**: 통합 동작
- ✅ **E2E Test**: 사용자 시나리오

### 기억하기

> "과도한 검증은 테스트의 적입니다"

- 각 레벨의 책임을 명확히 분리하세요
- 핵심만 검증하세요
- 유지보수 가능한 테스트를 작성하세요
- 테스트 코드도 프로덕션 코드만큼 중요합니다

---

## 참고 자료

- [NestJS Testing Documentation](https://docs.nestjs.com/fundamentals/testing)
- [Jest Documentation](https://jestjs.io/)
- [Test Pyramid - Martin Fowler](https://martinfowler.com/articles/practical-test-pyramid.html)

---

> **작성일**: 2025-11-23
> **버전**: 1.0
> **작성자**: Claude AI Assistant
