...
3. **페이지네이션 유틸리티**
   ...
   export class PaginationUtil {
      ...
      * 페이지네이션 결과 생성
      * @example
      * const result = PaginationUtil.paginate(users, totalCount, { page: 1, limit: 10 });
      */
     static paginate<T>(
       data: T[],
       total: number,
      ...
      * Skip 계산
      * @example
      * const skip = PaginationUtil.getSkip(1, 10); // returns 0
      * const skip = PaginationUtil.getSkip(2, 10); // returns 10
      */
...
**동작 순서**: 
 ...
 2. JwtAuthGuard.canActivate(): 헤더에서 토큰을 추출하여 서명 및 만료 시간 체크
 3. JwtStrategy.validate(): 토큰을 디코드하여 사용자 정보 검증(사용자 정의 검증)
 4. req.user: 검증된 사용자 정보를 req.user에 저장
 5. 컨트롤러에서는 @CurrentUser() 등을 통해 request.user에 접근
   ...
   export class JwtAuthGuard extends AuthGuard('jwt') {
     ...
     canActivate(context: ExecutionContext) {
       const isPublic = this.reflector.getAllAndOverride<boolean>(IS_PUBLIC_KEY, [
         context.getHandler(), // 메서드(핸들러) 수준 메타데이터
         context.getClass(), // 클래스 수준 메타데이터
       ]);
     ...
     /**
      * ✅ 선택적 인증이 필요한 경우 인증 결과 처리 메서드 handleRequest를 오버라이딩하고, 부모의 canActivate를 무조건 호출하여 인증 로직(Strategy)을 실행시켜야 함
      
      handleRequest(err, user, info, context: ExecutionContext) {
        // 사용자가 정상적으로 인증되었다면 그대로 반환 (req.user에 주입됨)
        if (user) {
          return user;
        }

        // ✅ 인증에 실패했거나 토큰이 없는 경우, Public 라우트인지 확인
        const isPublic = this.reflector.getAllAndOverride<boolean>(IS_PUBLIC_KEY, [
          context.getHandler(),
          context.getClass(),
        ]);

        // Public 라우트라면 에러를 던지지 않고 null을 반환 (로그인 안 한 상태로 통과)
        if (isPublic) {
          return null; 
        }

        // Public이 아닌데 인증도 안 됐다면 에러 발생 (기존 동작)
   ...
   export class JwtStrategy extends PassportStrategy(Strategy) {
     constructor(private configService: ConfigService) {
       super({
         jwtFromRequest: ExtractJwt.fromAuthHeaderAsBearerToken(), // 헤더에서 토큰을 추출
         ignoreExpiration: false, // 만료 시간을 체크
         secretOrKey: configService.get<string>('jwt.secret'), // 비밀키로 서명을 확인
       });
     }
      ...
      * 
      * ⚠️ 성능 고려사항:
      * 여기서는 RDB 접근을 지양합니다. 매 요청마다 DB를 조회하면 부하가 심해지기 때문입니다.
      * JWT의 Stateless 특성을 살려 Payload 검증만 수행합니다.
      * 
      * 만약 실시간 차단 여부 등 상태 확인이 꼭 필요하다면:
      * 1. Redis 캐시를 조회하거나
      * 2. 비즈니스 로직(Service) 단계에서 수행하는 것을 권장합니다.
      * 
      * 반환된 객체는 request.user에 주입됩니다.
      * @example
      * // request.user = { userId: 1, email: 'test@example.com', roles: ['USER'] }
      */
      ...
      imports: [
        ...
        JwtModule.registerAsync({
          ...
          useFactory: (config: ConfigService) => ({
            secret: config.get<string>('JWT_SECRET'),  // ✅ JWT 서명에 사용할 비밀키
            signOptions: {
              expiresIn: config.get<string>('JWT_ACCESS_EXPIRATION') || '1d',  // ✅ 액세스 토큰 만료 시간 (예: '1d', '7d')
            },
          }),
        }),
      ],
...
3. **보안 미들웨어 설정**
   ...
   async function bootstrap() {
     ...
     // 글로벌 프리픽스
     app.setGlobalPrefix(configService.get<string>('app.apiBasePath'));
     ...
     logger.log(
       `API prefix: ${configService.get<string>('app.apiBasePath')}`,
     );
   }
