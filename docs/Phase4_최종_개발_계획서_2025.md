# Phase 4 최종 개발 계획서 (2025)

**작성일**: 2025-11-18
**대상 기간**: Week 11-14 (4주)
**현재 상태**: Reviews 모듈 완료 → Messages/Notifications 구현 필요
**목표**: 확장 도메인 완성 + 시스템 통합 및 최적화

---

## 📋 현황 분석

### ✅ 완료된 모듈 (Phase 1-3 + Reviews)

| 모듈 | 상태 | API 엔드포인트 | E2E 테스트 | 특징 |
|------|------|---------------|-----------|------|
| Users | ✅ | 6개 | 13개 | JWT 인증, RBAC |
| Products | ✅ | 8개 | 21개 | 검색, 필터링, 상태 관리 |
| Categories | ✅ | 5개 | 11개 | 계층 구조 |
| Orders | ✅ | 8개 | 17개 | 상태 머신 패턴 |
| **Reviews** | ✅ | 7개 | 예상 10-15개 | 신뢰도 점수 시스템 |

**총계**: 5개 모듈, 34개 API, 약 72-77개 E2E 테스트

### 🏗️ Prisma 스키마 현황

**완전히 정의된 모델**:
- ✅ User (신뢰도 점수 필드 포함)
- ✅ Category (계층형 구조)
- ✅ Product (상태, 위치 정보)
- ✅ Order (결제, 배송 정보)
- ✅ Review (평점, 이미지)
- ✅ ChatRoom (채팅방)
- ✅ ChatRoomMember (멤버 관리)
- ✅ ChatMessage (메시지 타입별)
- ✅ Favorite (찜하기)
- ✅ Notification (알림 타입별)

**스키마 완성도**: 100% ✅

### 📦 아키텍처 현황

**완성된 인프라**:
- ✅ NestJS 11.x + TypeScript 5.7.3
- ✅ PostgreSQL + Prisma ORM 6.2.0
- ✅ MongoDB (확장 데이터)
- ✅ Redis (캐싱)
- ✅ RabbitMQ (이벤트 시스템)
- ✅ JWT 인증 (Access + Refresh)
- ✅ RBAC 권한 관리
- ✅ Winston 로깅
- ✅ Swagger API 문서
- ✅ Docker 환경

**보일러플레이트 완성도**: 100% ✅

---

## 🎯 Phase 4 목표 및 범위

### 최종 목표

1. **Messages 모듈 구현**: 구매자-판매자 1:1 채팅 시스템
2. **Notifications 모듈 구현**: 통합 알림 시스템 구축
3. **Favorites 모듈 구현**: 찜하기 기능 추가
4. **시스템 통합**: 전체 도메인 간 이벤트 연동
5. **코드 리팩토링**: 공통 로직 추출 및 최적화
6. **문서화**: API 문서 및 개발 가이드 최신화

### 성공 지표

- ✅ 3개 도메인 모듈 신규 완성 (Messages, Notifications, Favorites)
- ✅ 15-20개 API 엔드포인트 추가
- ✅ E2E 테스트 25-35개 추가 (총 97-112개)
- ✅ 전체 시스템 통합 테스트 통과
- ✅ 코드 리팩토링 완료
- ✅ 프로덕션 준비 완료

### 산출물 목록

1. ✅ Messages 모듈 (6-8개 API)
2. ✅ Notifications 모듈 (4-6개 API)
3. ✅ Favorites 모듈 (3-4개 API)
4. ✅ 이벤트 통합 시스템
5. ✅ 통합 테스트 스위트
6. ✅ 리팩토링된 코드베이스
7. ✅ 최신 API 문서
8. ✅ 개발 가이드 문서

---

## 📅 상세 개발 일정

### Week 11: Messages 모듈 구현 (5일)

#### 📌 Day 1: Repository 및 Service 구현

**오전: Repository 구현**

```typescript
// src/modules/messages/repositories/messages.repository.ts

@Injectable()
export class MessagesRepository {
  constructor(private readonly prisma: PrismaService) {}

  /**
   * 채팅방 생성 또는 조회
   * - 같은 상품에 대한 채팅방이 이미 있으면 기존 채팅방 반환
   */
  async findOrCreateChatRoom(userId: string, productId: string) {
    // 기존 채팅방 조회
    const existingRoom = await this.prisma.chatRoom.findFirst({
      where: {
        productId,
        members: {
          some: { userId },
        },
      },
      include: {
        members: {
          include: {
            user: {
              select: {
                id: true,
                nickname: true,
                profileImage: true,
              },
            },
          },
        },
      },
    });

    if (existingRoom) {
      return existingRoom;
    }

    // 상품 정보 조회 (판매자 확인)
    const product = await this.prisma.product.findUnique({
      where: { id: productId },
      select: { sellerId: true },
    });

    if (!product) {
      throw new NotFoundException('상품을 찾을 수 없습니다');
    }

    // 새 채팅방 생성 (구매자 + 판매자)
    return this.prisma.chatRoom.create({
      data: {
        productId,
        members: {
          create: [
            { userId }, // 요청자 (구매자)
            { userId: product.sellerId }, // 판매자
          ],
        },
      },
      include: {
        members: {
          include: {
            user: {
              select: {
                id: true,
                nickname: true,
                profileImage: true,
              },
            },
          },
        },
      },
    });
  }

  /**
   * 메시지 전송
   */
  async createMessage(
    senderId: string,
    chatRoomId: string,
    content: string,
    messageType: MessageType = MessageType.TEXT,
    fileUrl?: string,
    fileName?: string,
  ) {
    return this.prisma.chatMessage.create({
      data: {
        chatRoomId,
        senderId,
        content,
        messageType,
        fileUrl,
        fileName,
      },
      include: {
        sender: {
          select: {
            id: true,
            nickname: true,
            profileImage: true,
          },
        },
      },
    });
  }

  /**
   * 채팅방 목록 조회 (사용자별)
   */
  async findChatRoomsByUserId(userId: string) {
    return this.prisma.chatRoom.findMany({
      where: {
        members: {
          some: { userId },
        },
      },
      include: {
        members: {
          include: {
            user: {
              select: {
                id: true,
                nickname: true,
                profileImage: true,
              },
            },
          },
        },
        messages: {
          orderBy: { createdAt: 'desc' },
          take: 1, // 마지막 메시지 1개만
          include: {
            sender: {
              select: {
                id: true,
                nickname: true,
              },
            },
          },
        },
      },
      orderBy: { updatedAt: 'desc' },
    });
  }

  /**
   * 채팅방 메시지 히스토리 조회 (페이지네이션)
   */
  async findMessagesByRoomId(
    chatRoomId: string,
    page: number = 1,
    limit: number = 50,
  ) {
    const skip = (page - 1) * limit;

    const [messages, total] = await Promise.all([
      this.prisma.chatMessage.findMany({
        where: { chatRoomId },
        skip,
        take: limit,
        orderBy: { createdAt: 'desc' }, // 최신 메시지부터
        include: {
          sender: {
            select: {
              id: true,
              nickname: true,
              profileImage: true,
            },
          },
        },
      }),
      this.prisma.chatMessage.count({ where: { chatRoomId } }),
    ]);

    return {
      messages: messages.reverse(), // 시간순으로 정렬
      total,
      page,
      limit,
      totalPages: Math.ceil(total / limit),
    };
  }

  /**
   * 읽음 처리
   */
  async markMessagesAsRead(chatRoomId: string, userId: string) {
    // 상대방이 보낸 메시지만 읽음 처리
    await this.prisma.chatMessage.updateMany({
      where: {
        chatRoomId,
        senderId: { not: userId },
        isRead: false,
      },
      data: {
        isRead: true,
        readAt: new Date(),
      },
    });

    // 멤버의 lastReadAt 업데이트
    await this.prisma.chatRoomMember.updateMany({
      where: {
        chatRoomId,
        userId,
      },
      data: {
        lastReadAt: new Date(),
      },
    });
  }

  /**
   * 안읽은 메시지 수 조회
   */
  async getUnreadCount(chatRoomId: string, userId: string) {
    return this.prisma.chatMessage.count({
      where: {
        chatRoomId,
        senderId: { not: userId },
        isRead: false,
      },
    });
  }

  /**
   * 채팅방 상세 조회
   */
  async findChatRoomById(chatRoomId: string) {
    return this.prisma.chatRoom.findUnique({
      where: { id: chatRoomId },
      include: {
        members: {
          include: {
            user: {
              select: {
                id: true,
                nickname: true,
                profileImage: true,
              },
            },
          },
        },
      },
    });
  }

  /**
   * 채팅방 나가기
   */
  async leaveChatRoom(chatRoomId: string, userId: string) {
    await this.prisma.chatRoomMember.deleteMany({
      where: {
        chatRoomId,
        userId,
      },
    });

    // 멤버가 없으면 채팅방 삭제
    const remainingMembers = await this.prisma.chatRoomMember.count({
      where: { chatRoomId },
    });

    if (remainingMembers === 0) {
      await this.prisma.chatRoom.delete({
        where: { id: chatRoomId },
      });
    }
  }
}
```

**작업 항목**:
- [ ] MessagesRepository 클래스 생성
- [ ] 채팅방 생성/조회 로직 구현
- [ ] 메시지 CRUD 메서드 구현
- [ ] 읽음 처리 및 안읽은 메시지 카운트
- [ ] 페이지네이션 로직 구현

**오후: Service 구현**

```typescript
// src/modules/messages/messages.service.ts

@Injectable()
export class MessagesService {
  constructor(
    private readonly repository: MessagesRepository,
    private readonly productsService: ProductsService,
    private readonly eventEmitter: EventEmitter2,
  ) {}

  /**
   * 채팅방 생성 또는 조회
   *
   * 비즈니스 규칙:
   * 1. 상품이 존재해야 함
   * 2. 상품이 판매 가능 상태여야 함
   * 3. 본인 상품에는 채팅 불가
   */
  async findOrCreateChatRoom(userId: string, productId: string) {
    // 상품 존재 확인
    const product = await this.productsService.findById(productId);

    // 본인 상품 확인
    if (product.sellerId === userId) {
      throw new BadRequestException('본인 상품에는 채팅할 수 없습니다');
    }

    // 상품 상태 확인
    if (product.status === ProductStatus.DELETED) {
      throw new BadRequestException('삭제된 상품입니다');
    }

    const chatRoom = await this.repository.findOrCreateChatRoom(
      userId,
      productId,
    );

    // 새 채팅방 생성 시 알림
    if (chatRoom.isNew) {
      this.eventEmitter.emit('chatroom.created', {
        chatRoomId: chatRoom.room.id,
        productId,
        buyerId: userId,
        sellerId: product.sellerId,
      });
    }

    return chatRoom.room;
  }

  /**
   * 메시지 전송
   *
   * 비즈니스 규칙:
   * 1. 채팅방 멤버만 전송 가능
   * 2. 메시지 타입에 따라 검증 (파일, 이미지)
   */
  async sendMessage(
    userId: string,
    chatRoomId: string,
    content: string,
    messageType: MessageType = MessageType.TEXT,
    fileUrl?: string,
    fileName?: string,
  ) {
    // 채팅방 멤버 확인
    const chatRoom = await this.repository.findChatRoomById(chatRoomId);
    if (!chatRoom) {
      throw new NotFoundException('채팅방을 찾을 수 없습니다');
    }

    const isMember = chatRoom.members.some(
      (member) => member.userId === userId,
    );
    if (!isMember) {
      throw new ForbiddenException('채팅방 멤버만 메시지를 보낼 수 있습니다');
    }

    // 메시지 생성
    const message = await this.repository.createMessage(
      userId,
      chatRoomId,
      content,
      messageType,
      fileUrl,
      fileName,
    );

    // 채팅방 updatedAt 갱신
    await this.prisma.chatRoom.update({
      where: { id: chatRoomId },
      data: { updatedAt: new Date() },
    });

    // 상대방에게 알림 전송
    this.eventEmitter.emit('message.sent', {
      messageId: message.id,
      chatRoomId,
      senderId: userId,
      receiverId,
      content,
    });

    return message;
  }

  /**
   * 채팅방 목록 조회
   */
  async findMyChatRooms(userId: string) {
    const chatRooms = await this.repository.findChatRoomsByUserId(userId);

    // 각 채팅방의 안읽은 메시지 수 계산
    const chatRoomsWithUnread = await Promise.all(
      chatRooms.map(async (room) => {
        const unreadCount = await this.repository.getUnreadCount(
          room.id,
          userId,
        );

        return {
          ...room,
          unreadCount,
        };
      }),
    );
    return chatRoomsWithUnread;
  }

  /**
   * 채팅방 메시지 히스토리 조회
   */
  async findMessages(userId: string, chatRoomId: string, page: number = 1) {
    // 채팅방 멤버 확인
    const chatRoom = await this.repository.findChatRoomById(chatRoomId);
    if (!chatRoom) {
      throw new NotFoundException('채팅방을 찾을 수 없습니다');
    }

    const isMember = chatRoom.members.some(
      (member) => member.userId === userId,
    );
    if (!isMember) {
      throw new ForbiddenException('채팅방 멤버만 메시지를 조회할 수 있습니다');
    }

    // 메시지 조회
    return this.repository.findMessagesByRoomId(chatRoomId, page);
  }

  /**
   * 읽음 처리
   */
  async markAsRead(userId: string, chatRoomId: string) {
    // 채팅방 멤버 확인
    const chatRoom = await this.repository.findChatRoomById(chatRoomId);
    if (!chatRoom) {
      throw new NotFoundException('채팅방을 찾을 수 없습니다');
    }

    const isMember = chatRoom.members.some(
      (member) => member.userId === userId,
    );
    if (!isMember) {
      throw new ForbiddenException('권한이 없습니다');
    }

    await this.repository.markMessagesAsRead(chatRoomId, userId);
  }

  /**
   * 채팅방 나가기
   */
  async leaveChatRoom(userId: string, chatRoomId: string) {
    // 채팅방 멤버 확인
    const chatRoom = await this.repository.findChatRoomById(chatRoomId);
    if (!chatRoom) {
      throw new NotFoundException('채팅방을 찾을 수 없습니다');
    }

    const isMember = chatRoom.members.some(
      (member) => member.userId === userId,
    );
    if (!isMember) {
      throw new ForbiddenException('권한이 없습니다');
    }

    await this.repository.leaveChatRoom(chatRoomId, userId);
  }
}
```

**작업 항목**:
- [ ] MessagesService 클래스 생성
- [ ] 비즈니스 로직 구현 (권한 검증)
- [ ] 이벤트 발행 (message.sent, chatroom.created)
- [ ] 예외 처리 추가

---

#### 📌 Day 2: Controller 및 DTO 구현

**오전: DTO 작성**

```typescript
// src/modules/messages/dto/create-chatroom.dto.ts
export class CreateChatRoomDto {
  @ApiProperty({ description: '상품 ID' })
  @IsString()
  productId: string;
}

// src/modules/messages/dto/send-message.dto.ts
export class SendMessageDto {
  @ApiProperty({ description: '채팅방 ID' })
  @IsString()
  chatRoomId: string;

  @ApiProperty({ description: '메시지 내용' })
  @IsString()
  @IsNotEmpty()
  content: string;

  @ApiPropertyOptional({ description: '메시지 타입', enum: MessageType })
  @IsOptional()
  @IsEnum(MessageType)
  messageType?: MessageType;

  @ApiPropertyOptional({ description: '파일 URL' })
  @IsOptional()
  @IsString()
  fileUrl?: string;

  @ApiPropertyOptional({ description: '파일명' })
  @IsOptional()
  @IsString()
  fileName?: string;
}

// src/modules/messages/dto/query-messages.dto.ts
export class QueryMessagesDto {
  @ApiPropertyOptional({ description: '페이지 번호', default: 1 })
  @IsOptional()
  @Type(() => Number)
  @IsInt()
  @Min(1)
  page?: number = 1;

  @ApiPropertyOptional({ description: '페이지당 항목 수', default: 50 })
  @IsOptional()
  @Type(() => Number)
  @IsInt()
  @Min(1)
  @Max(100)
  limit?: number = 50;
}
```

**오후: Controller 구현**

```typescript
// src/modules/messages/messages.controller.ts

@ApiTags('messages')
@Controller('messages')
@UseGuards(JwtAuthGuard)
export class MessagesController {
  constructor(private readonly messagesService: MessagesService) {}

  /**
   * 채팅방 생성 또는 조회
   */
  @Post('chatrooms')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '채팅방 생성 또는 조회' })
  @ApiResponse({ status: 201, description: '채팅방 생성/조회 성공' })
  async createOrFindChatRoom(
    @CurrentUser('userId') userId: string,
    @Body() dto: CreateChatRoomDto,
  ) {
    return this.messagesService.findOrCreateChatRoom(userId, dto.productId);
  }

  /**
   * 내 채팅방 목록 조회
   */
  @Get('chatrooms')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '내 채팅방 목록 조회' })
  @ApiResponse({ status: 200, description: '채팅방 목록 조회 성공' })
  async getMyChatRooms(@CurrentUser('userId') userId: string) {
    return this.messagesService.findMyChatRooms(userId);
  }

  /**
   * 채팅방 메시지 히스토리 조회
   */
  @Get('chatrooms/:chatRoomId/messages')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '채팅방 메시지 조회' })
  @ApiResponse({ status: 200, description: '메시지 조회 성공' })
  async getMessages(
    @CurrentUser('userId') userId: string,
    @Param('chatRoomId') chatRoomId: string,
    @Query() query: QueryMessagesDto,
  ) {
    return this.messagesService.findMessages(
      userId,
      chatRoomId,
      query.page,
    );
  }

  /**
   * 메시지 전송
   */
  @Post()
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '메시지 전송' })
  @ApiResponse({ status: 201, description: '메시지 전송 성공' })
  async sendMessage(
    @CurrentUser('userId') userId: string,
    @Body() dto: SendMessageDto,
  ) {
    return this.messagesService.sendMessage(
      userId,
      dto.chatRoomId,
      dto.content,
      dto.messageType,
      dto.fileUrl,
      dto.fileName,
    );
  }

  /**
   * 읽음 처리
   */
  @Patch('chatrooms/:chatRoomId/read')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '메시지 읽음 처리' })
  @ApiResponse({ status: 200, description: '읽음 처리 성공' })
  @HttpCode(HttpStatus.OK)
  async markAsRead(
    @CurrentUser('userId') userId: string,
    @Param('chatRoomId') chatRoomId: string,
  ) {
    await this.messagesService.markAsRead(userId, chatRoomId);
    return { success: true };
  }

  /**
   * 채팅방 나가기
   */
  @Delete('chatrooms/:chatRoomId')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '채팅방 나가기' })
  @ApiResponse({ status: 204, description: '채팅방 나가기 성공' })
  @HttpCode(HttpStatus.NO_CONTENT)
  async leaveChatRoom(
    @CurrentUser('userId') userId: string,
    @Param('chatRoomId') chatRoomId: string,
  ) {
    await this.messagesService.leaveChatRoom(userId, chatRoomId);
  }
}
```

**작업 항목**:
- [ ] 3개 DTO 작성 (CreateChatRoom, SendMessage, QueryMessages)
- [ ] MessagesController 구현 (6개 엔드포인트)
- [ ] Swagger 문서화 데코레이터 추가
- [ ] MessagesModule 구성

---

#### 📌 Day 3-4: E2E 테스트 작성

**테스트 시나리오**:
1. ✅ 채팅방 생성 성공
2. ✅ 본인 상품에 채팅 시도 시 실패
3. ✅ 메시지 전송 성공
4. ✅ 채팅방 멤버만 메시지 전송 가능
5. ✅ 채팅방 목록 조회
6. ✅ 안읽은 메시지 수 계산
7. ✅ 메시지 히스토리 조회
8. ✅ 페이지네이션 동작 확인
9. ✅ 읽음 처리 성공
10. ✅ 채팅방 나가기 성공
11. ✅ 멤버가 모두 나가면 채팅방 삭제
12. ✅ 권한 없는 사용자 접근 차단

**예상 E2E 테스트**: 12-15개

---

#### 📌 Day 5: 이벤트 통합 및 알림 연동

**이벤트 리스너 구현**:

```typescript
// src/modules/notifications/listeners/message.listener.ts

@Injectable()
export class MessageListener {
  constructor(private readonly notificationsService: NotificationsService) {}

  @OnEvent('message.sent')
  async handleMessageSent(payload: {
    messageId: string;
    chatRoomId: string;
    senderId: string;
    receiverId: string;
    content: string;
  }) {
    // 수신자에게 알림 생성
    await this.notificationsService.create({
      userId: payload.receiverId,
      type: NotificationType.NEW_MESSAGE,
      title: '새 메시지',
      message: payload.content.substring(0, 50), // 미리보기
      relatedId: payload.chatRoomId,
      relatedType: 'chatroom',
    });
  }

  @OnEvent('chatroom.created')
  async handleChatRoomCreated(payload: {
    chatRoomId: string;
    productId: string;
    buyerId: string;
    sellerId: string;
  }) {
    // 판매자에게 알림 생성
    await this.notificationsService.create({
      userId: payload.sellerId,
      type: NotificationType.NEW_MESSAGE,
      title: '새 채팅 요청',
      message: '상품에 대한 채팅 요청이 도착했습니다',
      relatedId: payload.chatRoomId,
      relatedType: 'chatroom',
    });
  }
}
```

**작업 항목**:
- [ ] 메시지 이벤트 리스너 구현
- [ ] 알림 자동 생성 로직
- [ ] RabbitMQ 메시지 큐 통합 (선택적)

---

### Week 12: Notifications 모듈 구현 (3일)

#### 📌 Day 1: Repository 및 Service 구현

**Repository 구현**:

```typescript
// src/modules/notifications/repositories/notifications.repository.ts

@Injectable()
export class NotificationsRepository {
  constructor(private readonly prisma: PrismaService) {}

  /**
   * 알림 생성
   */
  async create(data: {
    userId: string;
    type: NotificationType;
    title: string;
    message: string;
    relatedId?: string;
    relatedType?: string;
  }) {
    return this.prisma.notification.create({
      data,
    });
  }

  /**
   * 알림 목록 조회 (페이지네이션)
   */
  async findMany(userId: string, page: number = 1, limit: number = 20) {
    const skip = (page - 1) * limit;

    const [items, total, unreadCount] = await Promise.all([
      this.prisma.notification.findMany({
        where: { userId },
        skip,
        take: limit,
        orderBy: { createdAt: 'desc' },
      }),
      this.prisma.notification.count({ where: { userId } }),
      this.prisma.notification.count({
        where: { userId, isRead: false },
      }),
    ]);

    return {
      items,
      total,
      page,
      limit,
      totalPages: Math.ceil(total / limit),
      unreadCount,
    };
  }

  /**
   * 읽음 처리
   */
  async markAsRead(id: string, userId: string) {
    return this.prisma.notification.updateMany({
      where: { id, userId },
      data: {
        isRead: true,
        readAt: new Date(),
      },
    });
  }

  /**
   * 전체 읽음 처리
   */
  async markAllAsRead(userId: string) {
    return this.prisma.notification.updateMany({
      where: { userId, isRead: false },
      data: {
        isRead: true,
        readAt: new Date(),
      },
    });
  }

  /**
   * 알림 삭제
   */
  async delete(id: string, userId: string) {
    return this.prisma.notification.deleteMany({
      where: { id, userId },
    });
  }

  /**
   * 안읽은 알림 수 조회
   */
  async getUnreadCount(userId: string) {
    return this.prisma.notification.count({
      where: { userId, isRead: false },
    });
  }
}
```

**Service 구현**:

```typescript
// src/modules/notifications/notifications.service.ts

@Injectable()
export class NotificationsService {
  constructor(
    private readonly repository: NotificationsRepository,
  ) {}

  /**
   * 알림 생성
   */
  async create(data: {
    userId: string;
    type: NotificationType;
    title: string;
    message: string;
    relatedId?: string;
    relatedType?: string;
  }) {
    return this.repository.create(data);
  }

  /**
   * 내 알림 목록 조회
   */
  async findMyNotifications(userId: string, page: number = 1) {
    return this.repository.findMany(userId, page);
  }

  /**
   * 읽음 처리
   */
  async markAsRead(userId: string, id: string) {
    const result = await this.repository.markAsRead(id, userId);

    if (result.count === 0) {
      throw new NotFoundException('알림을 찾을 수 없습니다');
    }
  }

  /**
   * 전체 읽음 처리
   */
  async markAllAsRead(userId: string) {
    await this.repository.markAllAsRead(userId);
  }

  /**
   * 알림 삭제
   */
  async delete(userId: string, id: string) {
    const result = await this.repository.delete(id, userId);

    if (result.count === 0) {
      throw new NotFoundException('알림을 찾을 수 없습니다');
    }
  }

  /**
   * 안읽은 알림 수 조회
   */
  async getUnreadCount(userId: string) {
    return this.repository.getUnreadCount(userId);
  }
}
```

**작업 항목**:
- [ ] NotificationsRepository 구현
- [ ] NotificationsService 구현
- [ ] 알림 생성 로직
- [ ] 읽음 처리 로직

---

#### 📌 Day 2: Controller 및 이벤트 리스너 구현

**Controller 구현**:

```typescript
// src/modules/notifications/notifications.controller.ts

@ApiTags('notifications')
@Controller('notifications')
@UseGuards(JwtAuthGuard)
export class NotificationsController {
  constructor(
    private readonly notificationsService: NotificationsService,
  ) {}

  /**
   * 내 알림 목록 조회
   */
  @Get()
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '내 알림 목록 조회' })
  async getMyNotifications(
    @CurrentUser('userId') userId: string,
    @Query('page') page: number = 1,
  ) {
    return this.notificationsService.findMyNotifications(userId, page);
  }

  /**
   * 안읽은 알림 수 조회
   */
  @Get('unread/count')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '안읽은 알림 수 조회' })
  async getUnreadCount(@CurrentUser('userId') userId: string) {
    const count = await this.notificationsService.getUnreadCount(userId);
    return { unreadCount: count };
  }

  /**
   * 알림 읽음 처리
   */
  @Patch(':id/read')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '알림 읽음 처리' })
  @HttpCode(HttpStatus.OK)
  async markAsRead(
    @CurrentUser('userId') userId: string,
    @Param('id') id: string,
  ) {
    await this.notificationsService.markAsRead(userId, id);
    return { success: true };
  }

  /**
   * 전체 알림 읽음 처리
   */
  @Patch('read-all')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '전체 알림 읽음 처리' })
  @HttpCode(HttpStatus.OK)
  async markAllAsRead(@CurrentUser('userId') userId: string) {
    await this.notificationsService.markAllAsRead(userId);
    return { success: true };
  }

  /**
   * 알림 삭제
   */
  @Delete(':id')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '알림 삭제' })
  @HttpCode(HttpStatus.NO_CONTENT)
  async delete(
    @CurrentUser('userId') userId: string,
    @Param('id') id: string,
  ) {
    await this.notificationsService.delete(userId, id);
  }
}
```

**이벤트 리스너 통합**:

```typescript
// src/modules/notifications/listeners/order.listener.ts

@Injectable()
export class OrderListener {
  constructor(private readonly notificationsService: NotificationsService) {}

  @OnEvent('order.created')
  async handleOrderCreated(payload: {
    orderId: string;
    buyerId: string;
    sellerId: string;
  }) {
    // 판매자에게 알림
    await this.notificationsService.create({
      userId: payload.sellerId,
      type: NotificationType.ORDER_CREATED,
      title: '새 주문',
      message: '상품에 대한 주문이 접수되었습니다',
      relatedId: payload.orderId,
      relatedType: 'order',
    });
  }

  @OnEvent('order.confirmed')
  async handleOrderConfirmed(payload: {
    orderId: string;
    buyerId: string;
    sellerId: string;
  }) {
    // 구매자에게 알림
    await this.notificationsService.create({
      userId: payload.buyerId,
      type: NotificationType.ORDER_CONFIRMED,
      title: '주문 확정',
      message: '주문이 확정되었습니다',
      relatedId: payload.orderId,
      relatedType: 'order',
    });
  }

  @OnEvent('order.cancelled')
  async handleOrderCancelled(payload: {
    orderId: string;
    buyerId: string;
    sellerId: string;
  }) {
    // 판매자에게 알림
    await this.notificationsService.create({
      userId: payload.sellerId,
      type: NotificationType.ORDER_CANCELLED,
      title: '주문 취소',
      message: '주문이 취소되었습니다',
      relatedId: payload.orderId,
      relatedType: 'order',
    });
  }

  @OnEvent('payment.completed')
  async handlePaymentCompleted(payload: {
    orderId: string;
    buyerId: string;
    sellerId: string;
  }) {
    // 판매자에게 알림
    await this.notificationsService.create({
      userId: payload.sellerId,
      type: NotificationType.PAYMENT_COMPLETED,
      title: '결제 완료',
      message: '주문에 대한 결제가 완료되었습니다',
      relatedId: payload.orderId,
      relatedType: 'order',
    });
  }
}

// src/modules/notifications/listeners/review.listener.ts

@Injectable()
export class ReviewListener {
  constructor(private readonly notificationsService: NotificationsService) {}

  @OnEvent('review.created')
  async handleReviewCreated(payload: {
    reviewId: string;
    reviewerId: string;
    reviewedId: string;
  }) {
    // 리뷰 대상자에게 알림
    await this.notificationsService.create({
      userId: payload.reviewedId,
      type: NotificationType.REVIEW_RECEIVED,
      title: '새 리뷰',
      message: '새로운 리뷰가 작성되었습니다',
      relatedId: payload.reviewId,
      relatedType: 'review',
    });
  }
}
```

**작업 항목**:
- [ ] NotificationsController 구현 (5개 엔드포인트)
- [ ] 주문 이벤트 리스너 구현
- [ ] 리뷰 이벤트 리스너 구현
- [ ] NotificationsModule 구성

---

#### 📌 Day 3: E2E 테스트 작성

**테스트 시나리오**:
1. ✅ 알림 목록 조회
2. ✅ 안읽은 알림 수 조회
3. ✅ 읽음 처리 성공
4. ✅ 전체 읽음 처리
5. ✅ 알림 삭제
6. ✅ 페이지네이션 동작
7. ✅ 이벤트 발행 시 자동 알림 생성
8. ✅ 다른 사용자의 알림 접근 차단

**예상 E2E 테스트**: 8-10개

---

### Week 13: Favorites 모듈 및 시스템 통합 (5일)

#### 📌 Day 1-2: Favorites 모듈 구현

**Repository 및 Service**:

```typescript
// src/modules/favorites/repositories/favorites.repository.ts

@Injectable()
export class FavoritesRepository {
  constructor(private readonly prisma: PrismaService) {}

  /**
   * 찜하기 추가
   */
  async create(userId: string, productId: string) {
    return this.prisma.favorite.create({
      data: { userId, productId },
      include: {
        product: {
          include: {
            category: true,
            seller: {
              select: {
                id: true,
                nickname: true,
                profileImage: true,
              },
            },
          },
        },
      },
    });
  }

  /**
   * 찜하기 목록 조회 (페이지네이션)
   */
  async findMany(userId: string, page: number = 1, limit: number = 20) {
    const skip = (page - 1) * limit;

    const [items, total] = await Promise.all([
      this.prisma.favorite.findMany({
        where: { userId },
        skip,
        take: limit,
        orderBy: { createdAt: 'desc' },
        include: {
          product: {
            include: {
              category: true,
              seller: {
                select: {
                  id: true,
                  nickname: true,
                  profileImage: true,
                },
              },
            },
          },
        },
      }),
      this.prisma.favorite.count({ where: { userId } }),
    ]);

    return {
      items,
      total,
      page,
      limit,
      totalPages: Math.ceil(total / limit),
    };
  }

  /**
   * 찜하기 여부 확인
   */
  async exists(userId: string, productId: string) {
    const favorite = await this.prisma.favorite.findUnique({
      where: {
        userId_productId: { userId, productId },
      },
    });

    return !!favorite;
  }

  /**
   * 찜하기 삭제
   */
  async delete(userId: string, productId: string) {
    return this.prisma.favorite.deleteMany({
      where: { userId, productId },
    });
  }
}
```

**Service**:

```typescript
// src/modules/favorites/favorites.service.ts

@Injectable()
export class FavoritesService {
  constructor(
    private readonly repository: FavoritesRepository,
    private readonly productsService: ProductsService,
    private readonly eventEmitter: EventEmitter2,
  ) {}

  /**
   * 찜하기 추가
   */
  async addFavorite(userId: string, productId: string) {
    // 상품 존재 확인
    const product = await this.productsService.findById(productId);

    // 본인 상품 확인
    if (product.sellerId === userId) {
      throw new BadRequestException('본인 상품은 찜할 수 없습니다');
    }

    // 중복 확인
    const exists = await this.repository.exists(userId, productId);
    if (exists) {
      throw new ConflictException('이미 찜한 상품입니다');
    }

    const favorite = await this.repository.create(userId, productId);

    // 판매자에게 알림 이벤트 발행
    this.eventEmitter.emit('product.liked', {
      favoriteId: favorite.id,
      userId,
      productId,
      sellerId: product.sellerId,
    });

    return favorite;
  }

  /**
   * 찜 목록 조회
   */
  async findMyFavorites(userId: string, page: number = 1, order: string = 'createdAt', sort: 'DESC' | 'ASC' = 'DESC') {
    return this.repository.findMany(userId, page, order, sort);
  }

  /**
   * 찜하기 여부 확인
   */
  async checkFavorite(userId: string, productId: string) {
    const isFavorite = await this.repository.exists(userId, productId);
    return { isFavorite };
  }

  /**
   * 찜하기 삭제
   */
  async removeFavorite(userId: string, productId: string) {
    const result = await this.repository.delete(userId, productId);

    if (result.count === 0) {
      throw new NotFoundException('찜한 상품이 아닙니다');
    }
  }
}
```

**Controller**:

```typescript
// src/modules/favorites/favorites.controller.ts

@ApiTags('favorites')
@Controller('favorites')
@UseGuards(JwtAuthGuard)
export class FavoritesController {
  constructor(private readonly favoritesService: FavoritesService) {}

  /**
   * 찜하기 추가
   */
  @Post()
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '찜하기 추가' })
  async addFavorite(
    @CurrentUser('userId') userId: string,
    @Body('productId') productId: string,
  ) {
    return this.favoritesService.addFavorite(userId, productId);
  }

  /**
   * 찜 목록 조회
   */
  @Get()
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '찜 목록 조회' })
  async getMyFavorites(
    @CurrentUser('userId') userId: string,
    @Query() query: QueryFavoritesDto,
  ) {
    return this.favoritesService.findMyFavorites(userId, page);
  }

  /**
   * 찜하기 여부 확인
   */
  @Get(':productId/check')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '찜하기 여부 확인' })
  async checkFavorite(
    @CurrentUser('userId') userId: string,
    @Param('productId') productId: string,
  ) {
    return this.favoritesService.checkFavorite(userId, productId);
  }

  /**
   * 찜하기 삭제
   */
  @Delete(':productId')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: '찜하기 삭제' })
  @HttpCode(HttpStatus.NO_CONTENT)
  async removeFavorite(
    @CurrentUser('userId') userId: string,
    @Param('productId') productId: string,
  ) {
    await this.favoritesService.removeFavorite(userId, productId);
  }
}
```

**E2E 테스트**: 5-8개

**작업 항목**:
- [ ] FavoritesRepository 구현
- [ ] FavoritesService 구현
- [ ] FavoritesController 구현 (4개 엔드포인트)
- [ ] E2E 테스트 작성
- [ ] 알림 이벤트 통합

---

#### 📌 Day 3: 전역 에러 처리 및 응답 변환

**전역 예외 필터**:

```typescript
// src/common/filters/http-exception.filter.ts

@Catch()
export class AllExceptionsFilter implements ExceptionFilter {
  constructor(private readonly logger: Logger) {}

  catch(exception: unknown, host: ArgumentsHost) {
    const ctx = host.switchToHttp();
    const response = ctx.getResponse<Response>();
    const request = ctx.getRequest<Request>();

    let status = HttpStatus.INTERNAL_SERVER_ERROR;
    let message = 'Internal server error';
    let error = 'Internal Server Error';

    if (exception instanceof HttpException) {
      status = exception.getStatus();
      const errorResponse = exception.getResponse();

      if (typeof errorResponse === 'string') {
        message = errorResponse;
      } else if (typeof errorResponse === 'object') {
        message = (errorResponse as any).message || message;
        error = (errorResponse as any).error || error;
      }
    } else if (exception instanceof Error) {
      message = exception.message;
    }

    // 로깅
    this.logger.error({
      timestamp: new Date().toISOString(),
      path: request.url,
      method: request.method,
      statusCode: status,
      message,
      error,
      stack: exception instanceof Error ? exception.stack : undefined,
    });

    // 표준 에러 응답 형식
    response.status(status).json({
      success: false,
      statusCode: status,
      timestamp: new Date().toISOString(),
      path: request.url,
      error,
      message,
    });
  }
}
```

**응답 변환 인터셉터**:

```typescript
// src/common/interceptors/transform.interceptor.ts

export interface Response<T> {
  success: boolean;
  statusCode: number;
  timestamp: string;
  data: T;
  message?: string;
}

@Injectable()
export class TransformInterceptor<T>
  implements NestInterceptor<T, Response<T>>
{
  intercept(
    context: ExecutionContext,
    next: CallHandler,
  ): Observable<Response<T>> {
    return next.handle().pipe(
      map((data) => ({
        success: true,
        statusCode: context.switchToHttp().getResponse().statusCode,
        timestamp: new Date().toISOString(),
        data,
      })),
    );
  }
}
```

**작업 항목**:
- [ ] AllExceptionsFilter 구현
- [ ] TransformInterceptor 구현
- [ ] main.ts에 전역 등록
- [ ] 테스트 검증

---

#### 📌 Day 4-5: 통합 테스트 및 리팩토링

**통합 테스트 시나리오**:

1. **전체 거래 플로우**:
   - 회원가입 → 상품 등록 → 찜하기 → 채팅 → 주문 → 결제 → 배송 → 리뷰

2. **이벤트 연동**:
   - 주문 생성 → 알림 자동 생성
   - 메시지 전송 → 알림 자동 생성
   - 리뷰 작성 → 신뢰도 점수 업데이트

3. **권한 검증**:
   - 역할별 접근 제어
   - 본인 데이터만 수정 가능

**리팩토링 목록**:

```typescript
// 공통 DTO 추출
// src/common/dto/pagination.dto.ts
export class PaginationDto {
  @ApiPropertyOptional({ default: 1 })
  @Type(() => Number)
  @IsInt()
  @Min(1)
  page?: number = 1;

  @ApiPropertyOptional({ default: 20 })
  @Type(() => Number)
  @IsInt()
  @Min(1)
  @Max(100)
  limit?: number = 20;
}

// 공통 응답 DTO
// src/common/dto/paginated-response.dto.ts
export class PaginatedResponseDto<T> {
  items: T[];
  total: number;
  page: number;
  limit: number;
  totalPages: number;

  constructor(partial: Partial<PaginatedResponseDto<T>>) {
    Object.assign(this, partial);
  }
}
```

**작업 항목**:
- [ ] 전체 거래 플로우 E2E 테스트
- [ ] 이벤트 연동 테스트
- [ ] 공통 DTO 추출
- [ ] 코드 중복 제거
- [ ] 타입 정의 최적화

---

### Week 14: 문서화 및 최종 검증 (5일)

#### 📌 Day 1-2: API 문서 최신화

**Swagger 문서 개선**:

```typescript
// src/main.ts에 Swagger 설정 강화

const config = new DocumentBuilder()
  .setTitle('중고거래 플랫폼 API')
  .setDescription('중고거래 플랫폼 백엔드 API 문서')
  .setVersion('1.0')
  .addBearerAuth(
    {
      type: 'http',
      scheme: 'bearer',
      bearerFormat: 'JWT',
      description: 'Access Token',
    },
    'access-token',
  )
  .addTag('users', '사용자 관리')
  .addTag('products', '상품 관리')
  .addTag('categories', '카테고리 관리')
  .addTag('orders', '주문 관리')
  .addTag('reviews', '리뷰 관리')
  .addTag('messages', '메시지/채팅')
  .addTag('notifications', '알림')
  .addTag('favorites', '찜하기')
  .build();

const document = SwaggerModule.createDocument(app, config);
SwaggerModule.setup('api/docs', app, document, {
  swaggerOptions: {
    persistAuthorization: true,
    tagsSorter: 'alpha',
    operationsSorter: 'alpha',
  },
});
```

**작업 항목**:
- [ ] 모든 엔드포인트 Swagger 문서화
- [ ] 예시 요청/응답 추가
- [ ] 에러 응답 문서화

---

#### 📌 Day 3: README 및 개발 가이드

**README.md 구조**:

```markdown
# 중고거래 플랫폼 백엔드 API

## 프로젝트 개요
- 기술 스택
- 주요 기능
- 아키텍처 다이어그램

## 시작하기
- 사전 요구사항
- 설치 방법
- 환경 설정
- 실행 방법

## API 문서
- Swagger UI 링크
- 주요 엔드포인트 설명

## 개발 가이드
- 프로젝트 구조
- 개발 패턴
- 코딩 컨벤션
- 테스트 가이드

## 배포
- Docker 빌드
- 환경별 설정
- CI/CD

## 라이선스
```

**작업 항목**:
- [ ] README.md 작성
- [ ] API 사용 가이드
- [ ] 개발 환경 설정 가이드
- [ ] 배포 가이드

---

#### 📌 Day 4-5: 최종 검증 및 릴리즈 준비

**체크리스트**:

1. ✅ 모든 E2E 테스트 통과 (목표: 97-112개)
2. ✅ 빌드 성공 (TypeScript strict 모드)
3. ✅ ESLint 0 에러
4. ✅ Prettier 포맷팅 일관성
5. ✅ API 문서 완성
6. ✅ README 작성
7. ✅ 환경 변수 검증
8. ✅ Docker 빌드 성공
9. ✅ 성능 테스트 (API 응답 시간 <200ms)
10. ✅ 보안 체크리스트

**릴리즈 노트 작성**:

```markdown
# Release Notes - Phase 4

## 버전: v1.0.0
## 릴리즈 날짜: 2025-XX-XX

### 새로운 기능
- Messages 모듈: 실시간 1:1 채팅 시스템
- Notifications 모듈: 통합 알림 시스템
- Favorites 모듈: 상품 찜하기 기능
- 전역 에러 처리 및 응답 변환
- 이벤트 기반 도메인 간 연동

### API 엔드포인트
- Messages: 6개 API
- Notifications: 5개 API
- Favorites: 4개 API

### 테스트
- E2E 테스트: 97-112개 (100% 통과)
- 통합 테스트: 전체 거래 플로우

### 기술 스택
- NestJS 11.x
- TypeScript 5.7.3
- Prisma ORM 6.2.0
- PostgreSQL
- Redis
- RabbitMQ

### 개선사항
- 공통 DTO 추출 및 재사용
- 코드 리팩토링 및 최적화
- API 문서 자동화
- 전역 에러 처리 표준화
```

---

## 📊 Phase 4 최종 산출물

### 모듈별 완성도

| 모듈 | API | E2E 테스트 | 상태 |
|------|-----|-----------|------|
| Users | 6 | 13 | ✅ |
| Products | 8 | 21 | ✅ |
| Categories | 5 | 11 | ✅ |
| Orders | 8 | 17 | ✅ |
| Reviews | 7 | 10-15 | ✅ |
| **Messages** | 6 | 12-15 | 📝 Week 11 |
| **Notifications** | 5 | 8-10 | 📝 Week 12 |
| **Favorites** | 4 | 5-8 | 📝 Week 13 |

**총계**: 8개 모듈, 49개 API, 97-110개 E2E 테스트

### 기술 지표

**코드 품질**:
- ✅ TypeScript Strict Mode
- ✅ ESLint 0 에러
- ✅ Prettier 일관된 포맷팅
- ✅ 100% 빌드 성공률

**테스트 커버리지**:
- ✅ E2E 테스트: 100% 통과
- ✅ 단위 테스트: 100% 통과
- ✅ 통합 테스트: 전체 플로우

**성능 목표**:
- ✅ API 응답 시간: <200ms
- ✅ 데이터베이스 쿼리: <100ms
- ✅ 동시 사용자: 1,000명
- ✅ 가용성: 99.9%

---

## 🔑 핵심 성공 요인

### 1. 완성된 인프라
- NestJS 모듈화 아키텍처
- Prisma ORM 타입 안전성
- JWT + RBAC 보안
- RabbitMQ 이벤트 시스템
- Winston 로깅
- Docker 환경

### 2. 검증된 개발 패턴
- Repository 패턴
- Service 패턴
- Controller 패턴
- 상태 머신 패턴
- 이벤트 기반 아키텍처

### 3. 체계적인 테스트
- E2E 테스트 100% 커버리지
- 통합 테스트 시나리오
- 권한 검증 테스트
- 엣지 케이스 테스트

### 4. 명확한 문서화
- Swagger API 자동 문서
- README 및 개발 가이드
- 릴리즈 노트
- 코드 주석

---

## 🚀 다음 단계 (Phase 5 예상)

### Week 15-16: 결제 및 고급 기능 (선택적)

1. **Payments 모듈**:
   - 토스페이먼츠 연동
   - 결제 상태 관리
   - 환불 처리

2. **성능 최적화**:
   - Redis 캐싱 전략
   - 데이터베이스 인덱스 최적화
   - N+1 문제 해결

3. **모니터링 강화**:
   - 메트릭 수집
   - 성능 대시보드
   - 알람 설정

4. **프로덕션 준비**:
   - 로드 테스트
   - 보안 감사
   - 배포 자동화

---

## 📚 참고 문서

1. [개발계획서 (메인)](./개발계획서_2025_10_24_업데이트.md)
2. [PRD 문서](./1.%20PRD_중고거래사이트_백엔드.md)
3. [Prisma 스키마](../prisma/schema.prisma)
4. [Phase 3 완료 보고서](메모리: session_checkpoint_phase3_progress)

---

**문서 버전**: v1.0
**최종 업데이트**: 2025-11-18
**작성자**: 개발팀
**다음 업데이트**: Week 11 종료 시
